...
function FighterCard({ fighter, photoUrl, uploading, saved, error, onUpload, onRemove, locale = 'fr' }: {
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (file && file.type.startsWith('image/')) onUpload(fighter, file)
  }

  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]
    if (file) onUpload(fighter, file)
  }
  ...
  return (
    <div
      className={`relative group border transition-all ${
        ...
        uploading ? 'border-gold-500/50 bg-octagon-700' :
